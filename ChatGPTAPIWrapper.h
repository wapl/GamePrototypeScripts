...
class MYGAME_API UChatGPTAPIWrapper : public UObject
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditAnywhere,BlueprintReadWrite,Category="OpenAI")
	float Temperature=0.51f;

	UPROPERTY(EditAnywhere,BlueprintReadWrite,Category="OpenAI")
	float TopP=1.0f;
private:
	void  OnResponseRecieved(FHttpRequestPtr Request,FHttpResponsePtr Response,bool bWasSuccessful,FonSuccessDelegate OnSuccess,FOnErrorDelegate OnError);
	UFUNCTION(BlueprintCallable,Category="OpenAI")
